...
parameters {
  ...
  vector[3] mu_lapse_logit;
  vector<lower=0>[3] sigma_lapse_logit;
...
transformed parameters {
  ...
  vector[3*P] r  = exp(mu_logr[cond_of] + sigma_logr[cond_of] .* logr_z);
  ...
  vector[3*P] p_lapse = inv_logit(mu_lapse_logit[cond_of] + sigma_lapse_logit[cond_of] .* lapse_z);
  ...
  vector[3*P] sigma_k = fmax(sqrt(pk .* (1 - pk) .* r), 0.05);
  vector[3*P] sigma_f = fmax(sqrt(pf .* (1 - pf) .* r), 0.05);
}
...
model {
...

  {
    ...
    vector[N_correct] p_lapse_trial = p_lapse[pc_correct];
...

  {
    ...
    vector[N_false] p_lapse_trial = p_lapse[pc_false];
