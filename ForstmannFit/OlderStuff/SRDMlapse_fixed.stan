...
parameters {
  ...
  vector<lower=0>[3*P] t0;

  vector[P] d;
  vector<lower=0.01>[P] r; // floored away from 0 - r=0 alone would zero out sigma_k/sigma_f
  vector<lower=0>[P] B;
...
model {
  ...
  p_lapse ~ beta(1, 10); // favors small lapse rates unless the data says otherwise
