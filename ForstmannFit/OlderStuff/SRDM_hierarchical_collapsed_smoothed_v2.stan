...
transformed data {
  ...
  // FIXED constant (not estimated) controlling how sharply the lapse/diffusion
  // blend transitions across t_raw = 0 - small relative to RT scale, just enough
  // to keep the gradient finite instead of a hard if/else discontinuity
...
transformed parameters {
  ...
  // linear predictors are capped at 15 before exponentiating (exp(15) ~ 3.3
  // million, far above any plausible value) so exp() can never overflow to
  // literal Inf during unadapted early exploration - that Inf was combining
  // with other terms to produce Inf/Inf = NaN, the remaining source of the
  // "log_mix: lambda2 is nan" / "Phi: x is nan" exceptions from the last run.
  vector[3*P] c  = mu_c[cond_of] + sigma_c[cond_of] .* c_z;
  vector[3*P] d  = exp(fmin(mu_logd[cond_of] + sigma_logd[cond_of] .* logd_z, 15));
  ...
  // gathered up to per-cell length wherever needed alongside c/d/B
  vector[3*P] r_rep = r[participant_of];

  vector[3*P] pk = fmin(fmax(Phi(d/2 - c), 1e-6), 1 - 1e-6);
  vector[3*P] pf = fmin(fmax(Phi(-d/2 - c), 1e-6), 1 - 1e-6);
...
model {
...

  {
    ...
    vector[N_correct] p_lapse_trial = p_lapse[pid_correct]; // collapsed - gathered by participant, not cell
    ...
    // smooth blend instead of a hard if/else: 'weight' is ~1 well above the
    // t0 crossing (genuine decision), ~0 well below it (forced lapse), and
    // transitions gradually near zero rather than jumping - keeps the
    // gradient finite everywhere, avoiding the divergence-inducing kink
    // smooth blend instead of a hard if/else. Using log_inv_logit directly
    // (rather than log(inv_logit(x)) / log1m(inv_logit(x))) avoids ever forming
    // an intermediate probability that saturates to exact 0/1 in floating point -
    // that saturation was producing -inf derivatives feeding into 0 * -inf = NaN
    // gradients, which is what crashed initialization.
    // Identity used: 1 - inv_logit(x) = inv_logit(-x)
...

  {
    ...
    vector[N_false] p_lapse_trial = p_lapse[pid_false]; // collapsed - gathered by participant, not cell
