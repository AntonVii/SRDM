...
model {
...

  {
    ...
    // smooth blend instead of a hard if/else. log_inv_logit avoids ever
    // forming an intermediate probability that saturates to exact 0/1,
    // which was producing -inf derivatives -> 0 * -inf = NaN gradients.
...
generated quantities {
  ...
  // Uses the SAME smoothed blend as the model block so LOO scores the model
  // that was actually fit, not a different (hard-branch) likelihood.
