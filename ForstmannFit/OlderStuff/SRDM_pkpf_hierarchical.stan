data {
  ...
  // t0 is shared across a participant's 3 conditions, so its bound is
  // computed by pooling ALL of that participant's trials, not per-cell
  vector<lower=0>[P] t0_hi;     // 5th-percentile RT per participant (all conditions pooled)
}
...
parameters {
  // ---- pk/pf directly, hierarchical per-condition (replaces c and d') ----
  // pk: "match"/correct-favoring probability; pf: "nonmatch"/error-favoring probability
  vector[3] mu_pk_logit;
  vector<lower=0>[3] sigma_pk_logit;

  vector[3] mu_pf_logit;
  vector<lower=0>[3] sigma_pf_logit;

  // ---- B: per-condition, as in the current best model ----
  vector[3] mu_logB;
...
transformed parameters {
  ...
  // pk/pf built directly via non-centered logit-normal transforms - no c/d'
  // computed at all; clipped away from exact 0/1 the same way as before
  vector[3*P] pk = fmin(fmax(inv_logit(mu_pk_logit[cond_of] + sigma_pk_logit[cond_of] .* pk_z), 1e-6), 1 - 1e-6);
  vector[3*P] pf = fmin(fmax(inv_logit(mu_pf_logit[cond_of] + sigma_pf_logit[cond_of] .* pf_z), 1e-6), 1 - 1e-6);

  vector[3*P] B = exp(fmin(mu_logB[cond_of] + sigma_logB[cond_of] .* logB_z, 15));

  // collapsed: one value per participant, shared across their 3 conditions
  vector[P] r  = exp(fmin(mu_logr + sigma_logr * logr_z, 15));
  vector[P] t0 = t0_hi .* inv_logit(mu_t0_logit + sigma_t0_logit * t0_z);
  vector[P] p_lapse = inv_logit(mu_lapse_logit + sigma_lapse_logit * lapse_z);

  vector[3*P] r_rep  = r[participant_of];
  vector[3*P] t0_rep = t0[participant_of];
...
model {
  ...
  // ---- hyperpriors ----
  mu_pk_logit ~ normal(logit(0.85), 0.5); // reference doc: pm/pk typically 0.70-0.99
  sigma_pk_logit ~ exponential(3);

  mu_pf_logit ~ normal(logit(0.20), 0.5); // reference doc: pn/pf typically 0.05-0.55
  sigma_pf_logit ~ exponential(3);

  mu_logB ~ normal(log(1.0), 0.5);
  sigma_logB ~ exponential(3);

  mu_logr ~ normal(log(4.5), 0.5);
  sigma_logr ~ exponential(3);

  mu_t0_logit ~ normal(0, 1);
  sigma_t0_logit ~ exponential(2);

  mu_lapse_logit ~ normal(logit(0.02), 1);
  ...
  {
    vector[N_correct] t0_trial = t0_rep[pc_correct];
    ...
    for (i in 1:N_correct) {
      ...
      target += log_sum_exp(
        log_w   + log_mix(p_lapse_trial[i], lapse_lp[i], sdt_lp[i]),
        log_1mw + log(p_lapse_trial[i]) + lapse_lp[i]
      );
    }
  }
  ...
  {
    vector[N_false] t0_trial = t0_rep[pc_false];
...
generated quantities {
  // pointwise log-likelihood for LOO/WAIC comparison against the c/d' model
  vector[N_correct + N_false] log_lik;

  // back-transformed, interpretable c / d' for every cell, derived from pk/pf:
  //   d' = Phi^-1(pk) - Phi^-1(pf)
  //   c  = -(Phi^-1(pk) + Phi^-1(pf)) / 2
  vector[3*P] c_derived;
  vector[3*P] d_derived;

  {
    vector[3*P] z_pk = inv_Phi(pk);
    vector[3*P] z_pf = inv_Phi(pf);
    d_derived = z_pk - z_pf;
    c_derived = -(z_pk + z_pf) / 2;
  }
