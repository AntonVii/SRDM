data {
  ...
  vector<lower=0>[P] t0_hi; // 5th-percentile RT per cell - keeps t0 away from the t=0 singularity
}
...
parameters {
  ...
  vector<lower=0, upper=t0_hi>[P] t0;


  vector<lower=0.05>[P] r; // floored away from 0 - r near 0 alone would collapse sigma_k/sigma_f
  vector<lower=0>[3*P] B_rep;
...
transformed parameters {
  ...
  // floored: pk/pf can still saturate toward 0/1 under extreme d/c draws,
  // but sigma_k/sigma_f can now never actually reach zero, which is what
  // was causing the division blowup -> Inf -> Inf*0 = NaN pattern
...
model {
  ...
  B_rep ~ exponential(0.5);

  r ~ gamma(9,2);

  p_lapse ~ beta(0.1, 400); // favors small lapse rates unless the data says otherwise
  ...
  {
    vector[N_correct] t0_trial = t0[pid_correct];
  ...
  {
    vector[N_false] t0_trial = t0[pid_false];
