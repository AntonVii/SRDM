data {
  ...
  // t0 is now shared across a participant's 3 conditions, so its bound is
...
transformed data {
  ...
  // blend transitions across t_raw = 0
...
transformed parameters {
  ...
  // linear predictors capped at 15 before exponentiating (exp(15) ~ 3.3
  // million) so exp() can never overflow to literal Inf.
  ...
  vector[P] d  = exp(fmin(mu_logd + sigma_logd * logd_z, 15));
...
model {
  ...
  mu_t0_logit ~ normal(0, 1);        // centered on ~50% of each participant's t0_hi
  sigma_t0_logit ~ exponential(2);

  mu_lapse_logit ~ normal(logit(0.015), 0.3);  // was normal(logit(0.02), 1) - much tighter around ~1.5%
  sigma_lapse_logit ~ exponential(8);           // was exponential(2) - mean 0.125 instead of 0.5, discourages spread that pushes the population mean up via the logit-skew effect
