data {
  ...
  // t0 varies by condition again, so its bound is per-cell (5th percentile
  // RT within that participant x condition cell), not pooled per-participant
  vector<lower=0>[3*P] t0_hi;
}
...
transformed parameters {
  ...
  // partially-pooled per-cell parameters (c, t0 vary by condition)
  ...
  // linear predictors capped at 15 before exponentiating so exp() can never
  // overflow to literal Inf.
  vector[P] d = exp(fmin(mu_logd + sigma_logd * logd_z, 15));
  vector[P] B = exp(fmin(mu_logB + sigma_logB * logB_z, 15));
  vector[P] r = exp(fmin(mu_logr + sigma_logr * logr_z, 15));
  vector[P] p_lapse = inv_logit(mu_lapse_logit + sigma_lapse_logit * lapse_z);

  // gathered up to per-cell length wherever needed alongside c/t0
