...
// levels. See LBA_varying_verr.stan for the version where both
// accumulators' drift rates vary by difficulty.
//
...
parameters {
  ...
  real<lower=0> v_error;             // SHARED across all difficulty levels
...
model {
...

  {
    ...
    for (i in 1:N_correct) {
      ...
      real cdf_lose = lba_survival_integral(t_dec, b_trial[i], A, v_error, s);
      real prob_neg = Phi(-vc_trial[i]/s) * Phi(-v_error/s);
  ...
  }
  {
    ...
    for (i in 1:N_false) {
      ...
      real pdf_win = lba_pdf(t_dec, b_trial[i], A, v_error, s);
