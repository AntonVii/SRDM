// DDM for rr98, NO-LAPSE variant, with sv and sz varying by SAT condition.
// N_LEVELS is passed in as data — set it to 7, 11, 16, 33, whatever you want.
//
// Difference from DDM_rr98_nolapse.stan:
//   - sv and sz are now vector[2] (one value per SAT condition: speed,
//     accuracy), indexed the same way `a` already is via sat_of.
//   - Rationale: sv shapes the slow-error tail, sz shapes the fast-error
//     tail, and the speed/accuracy manipulation is specifically designed
//     to shift the balance between fast and slow errors. A single shared
//     sv/sz has to compromise between two conditions that may genuinely
//     need different values, which can push MAP toward a degenerate
//     compromise near 0 for both. Letting them vary by condition removes
//     that forced compromise.
//   - Priors reverted to the original, only mildly informative choices
//     (sv ~ normal(0.6, 0.3), sz ~ beta(2,8)) applied independently to
//     each condition -- the point of this variant is to let the data
//     separate the two conditions, not to fight the likelihood with a
//     tight prior.
...
parameters {
  ...
  vector<lower=0>[2] sv;             // now per SAT condition
  vector<lower=0, upper=0.9>[2] sz;  // now per SAT condition
...
transformed parameters {
  ...
  vector[N_CELLS] sv_full = sv[sat_of];
  vector[N_CELLS] sz_full = sz[sat_of];
}
...
model {
  ...
  // Correct trials
  {
    ...
    vector[N_correct] sv_trial = sv_full[cell_correct];
    vector[N_correct] sz_trial = sz_full[cell_correct];

    for (i in 1:N_correct) {
      real t_dec = fmax(rt_correct[i] - t0, 1e-4);
      vector[5] terms;
      for (kz in 1:5) {
        real w_node = 0.5 + (sz_trial[i]/2) * gl_nodes[kz];
        terms[kz] = gl_logw[kz]
          + bkg_log_density(t_dec, v_trial[i], sv_trial[i], a_trial[i], 1 - w_node, J);
      }
  ...
  // Error trials
  {
    ...
    vector[N_false] sv_trial = sv_full[cell_false];
    vector[N_false] sz_trial = sz_full[cell_false];
    ...
    for (i in 1:N_false) {
      ...
      for (kz in 1:5) {
        real w_node = 0.5 + (sz_trial[i]/2) * gl_nodes[kz];
        terms[kz] = gl_logw[kz]
          + bkg_log_density(t_dec, -v_trial[i], sv_trial[i], a_trial[i], w_node, J);
      }
      target += log_sum_exp(terms);
    }
  }
