data {
  ...
  // cell = (sat_id - 1) * 7 + difficulty_level, sat_id: 1=speed, 2=accuracy
  array[N_correct] int<lower=1,upper=14> cell_correct;
  array[N_false] int<lower=1,upper=14> cell_false;

  real<lower=0> max_rt;   // pooled lapse range (single value per participant)
  real<lower=0> t0_hi;   // t0 is scalar here - pooled across SAT
}

transformed data {
  real t0_blend_width = 0.03;

  // which SAT condition / difficulty level each of the 14 cells belongs to
  array[14] int sat_of;
  ...
  vector[14] is_accuracy;
  for (idx in 1:14) {
    is_accuracy[idx] = (sat_of[idx] == 2) ? 1.0 : 0.0;
  }
...
parameters {
  ...
  real delta_d; // free (unconstrained) SAT increment added to d_base in the accuracy condition
...
transformed parameters {
  ...
  vector[14] B_full = B[sat_of];
  vector[14] d_full = d_base[diff_of] + delta_d * is_accuracy;
...
model {
  ...
  delta_d ~ normal(0, 0.5); // weak, symmetric prior - lets data determine sign/magnitude
  c ~ normal(0, 0.5);
  B ~ exponential(1);
  r ~ gamma(9, 2);
  p_lapse ~ beta(1, 50);
